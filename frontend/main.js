...
const functionapi = process.env.FUNCTION_API;

const getVisitCount = () => {
    fetch(functionapi)
        .then((response) => response.text()) // Adjust to `.json()` if your API returns JSON
        .then((data) => {
            console.log("Raw response from API:", data);

            // Extract count from plain text (or adjust for JSON if needed)
